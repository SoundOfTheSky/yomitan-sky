...
export function generateKanji(dictionary: YomitanDictionary) {
  ...
  for (const item of dictionary.kanji) {
    ...
    if (wkItem) {
      item[1] = [...new Set([...wkItem.readings
        .filter(r => r.type === 'onyomi')
        .sort((a, b) => (a.primary ? 0 : 1) - (b.primary ? 0 : 1))
        .map(x => x.reading), ...item[1].split(' ')])].join(' ')
      item[2] = [...new Set([...wkItem.readings
        .filter(r => r.type === 'kunyomi')
        .sort((a, b) => (a.primary ? 0 : 1) - (b.primary ? 0 : 1))
        .map(x => x.reading), ...item[2].split(' ')])].join(' ')
      item[4] = [
        ...new Set([...wkItem.meanings
          .sort((a, b) => (a.primary ? 0 : 1) - (b.primary ? 0 : 1))
          .map(x => x.meaning),
        ...item[4],
        ].map(x => x.toLowerCase())),
...
const clearWKText = (text: string) => text
  .replaceAll('<radical>', '{')
  .replaceAll('</radical>', '}')
  .replaceAll('<kanji>', '[')
  .replaceAll('</kanji>', ']')
