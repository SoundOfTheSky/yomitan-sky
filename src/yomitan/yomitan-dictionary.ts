...
import { DictionaryIndex, DictionaryKanjiBankV3, DictionaryKanjiMetaBankV3, DictionaryTagBankV3, DictionaryTermBankV3, DictionaryTermMetaBankV3 } from './types'
...
export default class YomitanDictionary {
  ...
  public async merge(name: string, url: string, download: boolean) {
    ...
    if (download) {
      ...
      await file(`${path}.zip`).write(await fetch(url))
      await $`unzip ${path}.zip -d ${path}`
    }
    console.log(`Merging ${name}...`)
    for (const bank of BANKS)
      await this.mergeBanks(name, bank)
    ...
    const index = await file(Path.join(path, 'index.json')).json() as DictionaryIndex
    this.index.attribution ??= ''
    this.index.attribution += `\n\n=== ${name} ===\nLink: ${url}\n`
    if (index.attribution)
      this.index.attribution += index.attribution
  }
  ...
  private async mergeBanks(name: string, bankName: Bank) {
    ...
    for (let index = 1; ; index++)
      try {
        bank.push(...await file(`assets/${name}/${bankName}_bank_${index}.json`).json() as never[])
      }
      catch (error) {
  ...
  private async saveBank(bankName: Bank) {
    ...
    for (let index = 0; index < parts.length; index++)
      await file(Path.join('dist', this.index.title, `${bankName}_bank_${index + 1}.json`)).write(
